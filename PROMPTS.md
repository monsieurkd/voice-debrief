...
- `AGENTS.md` → `CLAUDE.md` → `KNOWLEDGE_BASE.md` are auto-loaded for agents
  working here — no need to repeat generalities, only deltas.
...
Keep the Digital Sanctuary tokens and ui.tsx primitives — no palette rebuild.
...
1. Run the adversarial-tester agent: attack it — guest paths, failed actions,
   mobile, edge inputs.
