...
("Digital Sanctuary": surgical refinement with sanctuary tokens preserved.)
...
- Prod build green (webpack; the in-worktree Turbopack `node_modules` symlink
  crash is a sandbox limitation, not a code issue; the isolated-copy build passed).

## Known follow-ups (out of makeover scope)
- `LoginPill.tsx` lacks a `className` prop so `cursor-pointer` can't be threaded
  in (zero impact because the UA gives links a pointer). Add a `className` prop in a
  follow-up if desired.
- Raw `＋` glyph still on the home-page CTA `src/app/page.tsx:56` (not in any
  leaf's file set so a future leaf can swap it for `IconPlus`).
