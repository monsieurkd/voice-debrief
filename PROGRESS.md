...
- **Review** (`/session/[id]`): editable doc with edit / add / delete(+undo) / reclassify. Edits write back `source='user'`, `was_corrected=true`.
- **See notes + plan** (`/`): Home = Tomorrow's plan (check-off) + Recent entries (mood/energy/overview cards).
- Extraction is Zod-validated with a 3-attempt retry; failure still persists the session so **the transcript is never lost** (the architecture's best decision, which held up under review).
...
```
Validated-LLM-call engine shared by extraction + interview: `src/lib/llm-call.ts` (`callJsonValidated` with json_object + Zod + retry).
...
- **Fallback (commented in `.env.local`):** Google Gemini free tier with `gemini-3.5-flash` / `gemini-3.1-flash-lite`.
- `glm-4.6` is a thinking model → `extract.ts` sets `max_tokens: 8000`; extraction is slowish (~30–90s).
- Swap providers (Z.ai / Gemini / OpenRouter / OpenAI / Ollama) through environment variables with no code changes. For SaaS: revisit whose API terms allow reselling (see Phase 3).
...
1. **Wire real tests**: add `npm test` running `extract-selftest` (the only script that actually asserts); convert `test-store/-writeback/-reclassify/-interview` to assert + exit non-zero (they print and exit 0 today, so they cannot fail CI); add unit tests for `dates.ts` (highest bug density, zero coverage).
...
4. **Abuse & spend caps ✅ per-user rate limits**: debrief 5/h, interview turns 20/h, samples 30/h, threads 5/h, demo week 3/h; keyed by user id and DB-backed. Open: daily spend budget, `/api/health`, Referer fix (still localhost; cosmetic for non-OpenRouter providers).
...
1. **Billing**: Stripe with free tier + paid (~$5–10/mo, anchored under Rosebud's $12.99); per-plan LLM caps enforcing Phase 1's budgets.
