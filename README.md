...
**Status:** voice-first conversational app with **voice in and out in every browser**
...
The pitch is not "get structured data." It is **a conversation with someone who
...
- **One fast model.** No large model and no JSON extraction round-trip; the reply
...
- **Multi-user with self-authorizing actions.** A signed session cookie
  (scrypt passwords, JWT via jose) gates routes via `src/proxy.ts`, while every
...
One-time Postgres setup (run as a superuser; local dev only, and pick a real
...
Providers are swapped through environment variables only. OpenAI, Z.ai (GLM), Google Gemini
...
```bash
npm test          # deterministic unit tests (node:test); no DB or API key needed
npm run typecheck
npm run lint
