...
**Status:** multi-user (email + password auth, per-user data isolation) — text
or a guided AI interview in → structured data out → an editable document → a
next-steps plan, exportable and deletable per session. Real-time voice/STT is
the next milestone.
